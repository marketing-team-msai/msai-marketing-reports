-- =====================================================================
-- UP 3/3: program-bucket rework
-- =====================================================================
-- Down version: 2026-09-02_program_row_type.down.sql
--
-- =====================================================================
-- BLOCKED. DO NOT RUN UNTIL THE LOVABLE CHANGE IS DEPLOYED.
-- =====================================================================
...
-- $9,237,904.98 to $11,690,160.95, and the chart grows a $2,452,256
-- "(multi)" bar plus two new zero bars.
--
-- That is the same trap this migration fixes inside f_pipeline_model,
-- arriving through the frontend instead. /overview must first filter to
-- rows where row_type is absent or equals 'program'. Written that way the
-- page behaves identically before and after this migration, so there is
-- no broken window between the two deploys.
-- =====================================================================
--
...
--      not move on either call /overview makes.
--
-- Two traps this migration handles
...
--   - DROP discards privileges. authenticated holds SELECT on every mktg
--     table and view, so the grants are re-issued below. Miss them and
--     the dashboard 403s on its next read.
--
-- No display strings are returned. row_type, program and the numbers go
-- back; the frontend composes the reconciling row's caption.
-- =====================================================================
...
--    Advertising reads 0 because paid touches are not captured, not
--    because paid produced nothing: 8 of its 12 Campaign Influence lists
--    are empty, and its only deal-touching campaign touches one deal that
--    sits outside the Net New pipeline. A bare $0.00 next to a media
--    budget line reads as "paid produces no pipeline", which is not what
--    the data says.
--
--    Config-driven rather than hardcoded in the function, so this is a
--    row to delete once the snap_ad_source leg is built, not a migration.
...
--    safe here and the existing privileges survive.
--
--    "where row_type = 'program'" is the entire purpose of the new
--    column. Without it this function silently absorbs the reconciling
--    row and sourced pipeline moves from $9,237,904.98 to
--    $11,690,160.95 on the close_year = null call.
...
-- ============================== PASTE 2 ==============================
-- Verification. Run separately, after PASTE 1. All four must pass.

-- (a) expect exactly 5 rows, in this order, no (unclassified):
...
-- (b) expect exactly one row: 148 | 9237904.98
select sum(sourced_deals) as deals, sum(sourced_pipeline) as pipeline
  from mktg.f_sourced_by_program(true, null)
 where row_type = 'program';

-- (c) expect exactly one row: 9237904.98 | 0.1847580996
select sourced_pipeline, sourced_share_of_need
  from mktg.f_pipeline_model(true, null);

-- (d) expect exactly one row: 5003533.00 | 0.10007066
--     THIS is the call /overview's Progress to 2027 Target tile makes.
--     Its numbers are not the same as (c) and both have to hold.
select sourced_pipeline, sourced_share_of_need
